...
| `BUG-003` | Bug | 2026-06-28 | User | Selected reporting period shows literal `{{ w.label }}` in the developer dashboard | Medium | `IN-PROGRESS` | [`BUG-003.md`](BUG-003.md) | [`features/patch-dashboard-window-label/`](../features/patch-dashboard-window-label/) — Patch Track (template-only): multiline Django variable tag isn't tokenized; join `{{ w.label }}` onto one line |
