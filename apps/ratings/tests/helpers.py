"""Shared builders for ratings tests — accounts, accepted apps, and curation evidence.

Kept in one place so every ratings test constructs fixtures identically: a user account, a
real **accepted** catalog app (the only kind the write path accepts, D-6), and — for the
gate's curated path — a real DIGEST impression recorded through the signals capture surface.
"""
...
def make_user(email: str = "rater@example.com") -> Account:
    return make_account(email)


def make_accepted_app(owner, *, tag_ids=None, name: str = "Demo App") -> App:
    """Submit and accept an app so it is catalogued (the rate-valid state, D-6)."""
...
def make_curated(user, app, *, when: datetime | None = None):
    """Give ``user`` a DIGEST impression of ``app`` — the curation the gate looks for (D-8)."""
    return capture.record_impression(
        user,
        app.id,
        surface=Surface.DIGEST,
        occurred_at=when or datetime(2026, 6, 1, 12, tzinfo=UTC),
    )
