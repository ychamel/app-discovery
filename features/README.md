...
Each feature that goes through the **Feature Track** pipeline gets **one folder** here,
named with a kebab-case slug (e.g. `weekly-digest/`, `quality-score-v1/`,
`submission-intake/`).

> **Patches live elsewhere.** Bug fixes, refactors, and chores run the **Patch Track** and
> live in their own [../patches/](../patches/) directory (with its own
> [INDEX.md](../patches/INDEX.md) and [README.md](../patches/README.md)) — kept separate so
> feature tracking stays uncongested. This README and [INDEX.md](INDEX.md) cover the
> Feature Track only.
...
> The **Patch Track** layout lives in [../patches/README.md](../patches/README.md).
...
> Creating a **patch** instead? See [../patches/README.md](../patches/README.md) — patch
> folders are created under `patches/`, not here.

Folders are never deleted on completion — a shipped feature's artifacts are the record of what was built and why. Mark its `Stage` as `done` in `CONTROL.md` instead.
