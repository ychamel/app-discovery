...
| DN-PS-DESIGN | Ratify the Stage-2 architecture: (1) host = **Render** (free now, ~$14–25/mo durable, PS-1); (2) email = **Resend** vs Postmark (PS-2); (3) consolidate the 6 `base.html` into **one shared responsive shell** vs in-place polish. Standard items PS-DESIGN-2/3/5/6/8 proceed on approval. | Software Architect | 2-design | **OPEN** — raised 2026-06-27; full rationale in [DESIGN.md](DESIGN.md) §15 + gate block. Architect stopped at the gate (one persona/session). External accounts + spend + a scope-shape call → user's to confirm before Stage 3. |
