...
| **Magic-link auth** (`accounts.auth_backend` + `LoginToken`) | Issue, deliver, and verify single-use email tokens; create sessions on success. | `issue_login_link(account, purpose, *, base_url, email_sender=None)`, `verify_token(raw) -> Account \| None`. *(Signature refined in build — see DL-5: caller resolves account existence / enumeration policy.)* | Token hashing, TTL, single-use consumption. |
