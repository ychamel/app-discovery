...
| [app-subscriptions](app-subscriptions/) | 2 User loop | `4-build` | 2026-06-21 | H1 (feeds H3) | app-pages, identity-accounts, signal-capture | _active — [TASKS.md](app-subscriptions/TASKS.md) written (DESIGN approved DN-14): **8 ordered tasks, all S/M, no `L`**. New app `apps/subscriptions/` owning one mutable table `subscriptions_subscription` (near-twin of `apps/ratings/`) with three deliberate contrasts: deletion CASCADEs (AS-5/AC9), the follow-write + its one `subscribe` emit are one atomic transaction (M5 1:1 by construction), and an empty-until-producer notice seam (AS-3=A). Spine: additive bulk `catalog.get_catalogued_apps(ids)` (T-01) → CASCADE `Subscription` store (T-02) → atomic write path (T-03) → read path (T-04) → notice seam (T-05) → views+feed+include (T-06) → `{% app_follow %}` slot edit (T-07) → admin/docs (T-08). Full AC1–AC9 coverage. Now with Senior Engineer to build (T-01 first; produce `TEST_PLAN.md`)_ |
