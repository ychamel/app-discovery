...
| [interest-profile](interest-profile/) | 2 User loop | `3-plan` | 2026-06-22 | H1 | identity-accounts, interest-taxonomy | _**active** — Phase-2 personalization substrate: users declare interest-taxonomy tags, the user side of the Ring-0 match (H1). [DESIGN.md](interest-profile/DESIGN.md) **ratified (DN-16 approved)** — new app `apps/interests/`, one mutable membership table, no parent row (empty=default), no D-7 emit (IP-5), §7 preserve-on-edit for AC7, single `declared_tag_ids` read (AC8); reuses D-3/D-4/D-5, no new global ADR. Planner decomposing into TASKS.md_ |
