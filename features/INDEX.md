...
| [interest-profile](interest-profile/) | 2 User loop | `5-release` | 2026-06-22 | H1 | identity-accounts, interest-taxonomy | _**active** — Phase-2 personalization substrate: users declare interest-taxonomy tags, the user side of the Ring-0 match (H1). **Built (T-01…T-06): new app `apps/interests/`** owning one mutable table `interests_interest` (one row per user×`Tag.id`, **no parent row** → empty=structural default AC6, **CASCADE** user FK AC9, `interests/0001` reversible). Single write path `set_interests`/`clear_interests` with the §7 set-replace **preserve-on-edit** reconcile (AC4 × AC7/M5=0) + all-or-nothing `is_valid_tag` validation (AC2) + **no `signals.capture` import** (IP-5). Single read surface `declared_tag_ids` (the matcher contract, AC8). Thin picker/save/clear views + activation include + the fail-soft `{% interest_prompt %}` nudge on `profile.html`. Read-only admin + README/CODEMAP/[TEST_PLAN.md](interest-profile/TEST_PLAN.md). **616 tests green (+64), ruff/check/no drift.** Reuses D-3/D-4/D-5, no new global ADR. Handed to Release Engineer._ |
