...
| [ratings-reviews](ratings-reviews/) | 2 User loop | `5-release` | 2026-06-21 | H1, H3 | app-pages, signal-capture | _active — **build complete** (Senior Engineer, T-01…T-08): new `apps/ratings/` app, own mutable `ratings_rating` store, recorded curated gate (`CURATED_SURFACES={DIGEST}`=D-8, fail-closed), single write/read paths (no average — AC6), thin views + `ratings/` include, AP-1 slot filled (fail-soft inclusion tag). **486 tests green** (+69), `ruff`/`check` clean, no drift; 2 additive reversible migrations rehearsed. [TEST_PLAN.md](ratings-reviews/TEST_PLAN.md) covers AC1–AC9. Release Engineer next_ |
