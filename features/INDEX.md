...
| [interest-profile](interest-profile/) | 2 User loop | `6-post-release` | 2026-06-22 | H1 | identity-accounts, interest-taxonomy | _**released local/dev** — Phase-2 personalization substrate: users declare interest-taxonomy tags, the user side of the Ring-0 match (H1). New app `apps/interests/` owning one mutable table `interests_interest` (one row per user×`Tag.id`, **no parent row** → empty=structural default AC6, **CASCADE** user FK AC9, `interests/0001` reversible). Single write path `set_interests`/`clear_interests` with the §7 set-replace **preserve-on-edit** reconcile (AC4 × AC7/M5=0) + all-or-nothing `is_valid_tag` validation (AC2) + **no `signals.capture` import** (IP-5). Single read surface `declared_tag_ids` (the matcher contract, AC8). Thin picker/save/clear views + the **two-part activation switch** (`config/urls` include + the `{% interest_prompt %}` line on `profile.html`, no flag). **[RELEASE_NOTES.md](interest-profile/RELEASE_NOTES.md) written; rollout→rollback rehearsed** up→down→up on a throwaway PostgreSQL DB. **616 tests green (+64), ruff/check/no drift.** Reuses D-3/D-4/D-5, no new global ADR. Advanced to `6-post-release` (outcome review expected to defer — no prod target/traffic + matcher unbuilt, as the seven prior closed-outs)._ |
