...
| [ratings-reviews](ratings-reviews/) | 2 User loop | `4-build` | 2026-06-21 | H1, H3 | app-pages, signal-capture | _active — [TASKS.md](ratings-reviews/TASKS.md) written: 8 ordered tasks (5×S/M split, no `L`), risk-front-loaded on the signals coupling (T-01 `has_impression`+index) + the gate (T-03), AP-1 slot edit isolated to T-07. Full AC1–AC9 coverage. Signal read surface → own `Rating` store → gate (D-8) → write path → display read → views+include → slot fill → admin/docs. Senior Engineer next_ |
