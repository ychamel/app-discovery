...
| [open-search-browse](open-search-browse/) | 2 User loop | `5-release` | 2026-06-23 | enabler / H3 | app-pages | _**active** — Phase-2 open discovery surface: browse / keyword-search / interest-filter the D-6 accepted-app catalogue → app-page, **money never buys position** (vision §4.1). [DESIGN.md](open-search-browse/DESIGN.md) **APPROVED** (DN-18) → [TASKS.md](open-search-browse/TASKS.md) **BUILT** T-01…T-07 (**676 tests green, +60**): additive `catalog_app.accepted_at`+`search_vector` (+indexes, `django.contrib.postgres`), write-path maintenance (`_search_vector_expr`), backfill migration, reverse-resolution `taxonomy.tag_ids_resolving_to` (AC3), the paginated DB-pushed `catalog.search_catalogue -> CatalogPage` (constant-query-count + ORDER-BY-neutrality assertions, AC9/AC5), model-less `apps/discovery/` + the `discover/` include (no `signals` import = AC6). [TEST_PLAN.md](open-search-browse/TEST_PLAN.md) maps AC1–AC9. Reuses D-3/D-5/D-6/D-7/D-8 — no new global ADR. Handed to **Release Engineer**_ |
