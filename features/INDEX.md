...
| [ratings-reviews](ratings-reviews/) | 2 User loop | `6-post-release` | 2026-06-21 | H1, H3 | app-pages, signal-capture | _active — **released local/dev** (Release Engineer): `apps/ratings/` shipped per [DESIGN §12](ratings-reviews/DESIGN.md). [RELEASE_NOTES.md](ratings-reviews/RELEASE_NOTES.md) written (gate-based promotion, metric→signal→alert map, two-line activation switch). **Rollout→rollback rehearsed** on a throwaway Postgres DB — `ratings/0001` + `signals/0003` apply and reverse cleanly (up→down→up). **486 tests green**, `ruff`/`check` clean, no drift. Outcome review expected to defer (no prod target/traffic — mirrors the five prior close-outs). Retrospective Analyst next_ |
