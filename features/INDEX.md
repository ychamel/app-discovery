...
| [app-subscriptions](app-subscriptions/) | 2 User loop | `1-define` (awaiting DN-13) | 2026-06-21 | H1 (feeds H3) | app-pages, identity-accounts, signal-capture | _active — [FEATURE_BRIEF.md](app-subscriptions/FEATURE_BRIEF.md) DRAFTED, awaiting approval (DN-13): the user-side follow loop (follow/unfollow + a followed-apps return feed + emit one D-7 `subscribe` event per follow) net-new from signal-capture SC-7/SC-8 (OQ-4) — generates the on-platform return/retention signal the corpus measures; unblocks developer-updates. Reuses D-3/D-6/D-7, no new global decision_ |
