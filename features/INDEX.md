...
| [interest-profile](interest-profile/) | 2 User loop | `4-build` | 2026-06-22 | H1 | identity-accounts, interest-taxonomy | _**active** — Phase-2 personalization substrate: users declare interest-taxonomy tags, the user side of the Ring-0 match (H1). [DESIGN.md](interest-profile/DESIGN.md) ratified (DN-16). [TASKS.md](interest-profile/TASKS.md) decomposed it into **6 ordered S/M tasks** (no `L`): T-01 `Interest` CASCADE store → T-02 single write path + §7 preserve-on-edit reconcile + config + metrics → T-03 `declared_tag_ids` read surface (AC8) → T-04 thin picker/save/clear views + activation include → T-05 `{% interest_prompt %}` onboarding nudge → T-06 admin/docs. Full AC1–AC9 + design-element coverage; reuses D-3/D-4/D-5, no new global ADR. Handed to Senior Engineer (build T-01 first, produce TEST_PLAN.md)._ |
