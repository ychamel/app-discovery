...
| [app-subscriptions](app-subscriptions/) | 2 User loop | `2-design` | 2026-06-21 | H1 (feeds H3) | app-pages, identity-accounts, signal-capture | _active — [FEATURE_BRIEF.md](app-subscriptions/FEATURE_BRIEF.md) APPROVED (DN-13: brief + AS-3=option A ship the empty-until-producer notice surface now + AS-5 deletion posture). The user-side follow loop (follow/unfollow + a followed-apps return feed + a forward-compatible notice surface + emit one D-7 `subscribe` event per follow) net-new from signal-capture SC-7/SC-8 (OQ-4) — generates the on-platform return/retention signal the corpus measures; unblocks developer-updates. Reuses D-3/D-6/D-7, no new global decision. Now with Software Architect for [DESIGN.md](app-subscriptions/DESIGN.md)_ |
